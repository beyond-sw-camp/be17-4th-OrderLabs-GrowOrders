...
오픈소스 기반이라 커뮤니티와 문서가 풍부해 문제 해결과 커스터마이징에도 유리하다는 점에서 Jenkins를 선택하게 되었습니다.  
...
백엔드에 Blue-Green 배포, 프론트엔드에 Canary 배포를 적용했는데, 이런 활용하려면 **Ingress Controller**가 꼭 필요했습니다.   
HTTPS 인증서를 따로 적용하지 않았지만, 추후 서비스를 정식으로 오픈할 때는 **보안 강화**를 위해 **HTTPS 인증을 도입할 계획**입니다.   
