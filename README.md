...
이 방식은 기존 버전(Blue)과 신규 버전(Green)을 동시에 준비한 뒤, **트래픽을 한 번에 전환하는 구조**이므로 사용자는 배포 중에도 끊김이나 불안정성을 체감하지 않습니다. 
...
따라서 Ansible을 활용함으로써 우리 조는 쿠버네티스 노드 초기 설정과 Jenkins 배포 과정에서 **인프라 전반의 안정성과 운영 효율성**을 확보할 수 있었습니다.
...
특히 우리는 백엔드에 Blue-Green 배포, 프론트엔드에 Canary 배포를 적용했는데, 이런 #=**무중단 배포 전략을 제대로 활용하려면 Ingress Controller가 꼭 필요**했습니다. 
